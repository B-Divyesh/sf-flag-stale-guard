...
`public/cli-demo-recording.svg` is an original, self-hosted terminal-session illustration derived from the released `flag-stale-guard demo` output. Its warm night-paper terminal palette and pressed-leaf green/red status accents belong to the field-guide system. It is not generated art and contains no third-party material; the paired HTML transcript is regression-tested against a packaged CLI run, with only the per-run temporary directory redacted.
