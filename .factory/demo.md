...
- **Website:** open `/?demo=1`, `/demo`, or click **Try it with sample data**. It displays three fixed flags from `examples/`, including an expired `legacy-cart` flag with two source references. The persistent banner says that nothing is saved. You can mark one sample source reference as reviewed; **Reset demo** restores the original three flags and announces the reset. Leaving the demo also discards that review state. Demo review state exists only in page memory, so it never reads, writes, or shares real data or browser storage.
