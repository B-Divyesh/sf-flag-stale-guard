...
The command creates a temporary sample workspace and prints where it is. Nothing in your repository changes. The landing page has a self-hosted terminal recording and transcript of this command. The website demo is at `/?demo=1` and uses the same sample data. It keeps its review state in memory. Reset demo or leaving the demo discards it.
